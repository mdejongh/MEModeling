...
{
    my $self = shift;
    my($BuildMEModelParams) = @_;

    my @_bad_arguments;
    (ref($BuildMEModelParams) eq 'HASH') or push(@_bad_arguments, "Invalid type for argument \"BuildMEModelParams\" (value was \"$BuildMEModelParams\")");
    if (@_bad_arguments) {
    ...
    my $model_ref = $BuildMEModelParams->{model_ref};
    my $workspace = $BuildMEModelParams->{workspace};
    my $output_id = $BuildMEModelParams->{output_id};
    
    # debugging
    my $rxn_start = $BuildMEModelParams->{rxn_start} || 0;
    my $num_rxns = $BuildMEModelParams->{num_rxns} || 1e6;
    my $count = 0;
    ...
    my @unassigned_codons;
    my $last_chance_codon;
    foreach my $aa_name (keys %{$genetic_code{$assigned_code}}) {
	foreach my $codon (keys %{$genetic_code{$assigned_code}{$aa_name}}) {
	    if (! exists $code_tRNA{$codon}) {
		if (exists $default_codon_assignment{$codon}) {
		    print STDERR "\t assigning $default_codon_assignment{$codon}{trna} for '$aa_name' and '$codon'\n";
		    $code_tRNA{$codon}{trna} = $default_codon_assignment{$codon}{trna};
		    $code_tRNA{$codon}{aa} = $default_codon_assignment{$codon}{aa};
		}
		else {
		    print STDERR "No tRNA assignment for codon $codon [$aa_name] and NO DEFAULT ASSIGNMENT!\n";
		    push @unassigned_codons, $codon;
		}
	    }
	    if (exists $code_tRNA{$codon}) {
		$last_chance_codon = $codon;
	    }
	}
    }

    # assign a tRNA or else things break
    if (! defined $last_chance_codon) {
	my $msg = "Genome has no valid tRNA annotations; may need to be reannotated";
	Bio::KBase::Exceptions::KBaseException->throw(error => $msg,
							       method_name => 'build_me_model');
    }
    foreach my $codon (@unassigned_codons) {
	print STDERR "Assigning $code_tRNA{$last_chance_codon}{trna} for $codon\n";
	$code_tRNA{$codon} = $code_tRNA{$last_chance_codon};
    }
...
#	push @{$reactions{$gene}}, "sink_$gene\_DNA_neu\tsink $gene\_DNA_neu\t1 $gene\_DNA_neu -->\treversible\tSinks\n";
	...
	my ${pi}=2*$sum_aa;
	my ${h}=2*$sum_aa+1;
	my $number_rib=int(($sum_aa+1)/17) || 1;
...
#	    my $trna2 = $genetic_code{$assigned_code}{$1}{$codons[0]};
    ...
    my %test_reactions = map { $_ => $reactions{$_} } (keys %m_genes, "Recycling");
