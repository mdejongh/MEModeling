package MEModeling::MEModelingServer;
...
our %return_counts = (
        'build_me_model' => 1,
        'build_me_model_async' => 1,
        'build_me_model_check' => 1,
        'version' => 1,
);

our %method_authentication = (
        'build_me_model' => 'required',
        'build_me_model_async' => 'required',
        'build_me_model_check' => 'required',
);
...
our %async_run_methods = (
        'build_me_model_async' => 'MEModeling.build_me_model',
);

our %async_check_methods = (
        'build_me_model_check' => 'MEModeling.build_me_model',
);
...
sub call_method {
    ...
    my $ctx = MEModeling::MEModelingServerContext->new($self->{loggers}->{userlog},
...
{
    # Service MEModeling requires authentication.
	...
	{
	    $self->exception('PerlError', "Authentication required for MEModeling but no authentication header was passed");
	}
    ...
    my @result;
    {
        ...
        my $stderr = MEModeling::MEModelingServerStderrWrapper->new($ctx, $get_time);
        $ctx->stderr($stderr);
...
sub get_method
{
    ...
    {
	$self->exception('NoSuchMethod',
			 "'$method' is not a valid method in service MEModeling.");
    }
...
package MEModeling::MEModelingServerContext;

use strict;

=head1 NAME

MEModeling::MEModelingServerContext
...
package MEModeling::MEModelingServerStderrWrapper;
...
unless (caller) {
    ...
    my @dispatch;
    {
        ...
        my $obj = MEModeling::MEModelingImpl->new;
        push(@dispatch, 'MEModeling' => $obj);
    }
    my %headers = (
        "Authorization" => $token,
        "CLI" => "1"
    );
    my $server = MEModeling::MEModelingServer->new(
