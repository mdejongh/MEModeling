package MEModeling::MEModelingClient;
...
sub new
{
    ...
    my $self = {
	client => MEModeling::MEModelingClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
# Authentication: required
	...
	method => "MEModeling.build_me_model",
	params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(error => "Error invoking method build_me_model",
...
sub version {
    ...
    if ($result) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(
            error => "Error invoking method build_me_model",
            status_line => $self->{client}->status_line,
            method_name => 'build_me_model',
        );
    }
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for MEModeling::MEModelingClient\n";
    }
    if ($sMajor == 0) {
        warn "MEModeling::MEModelingClient version is $svr_version. API subject to change.\n";
    }
}
...
package MEModeling::MEModelingClient::RpcClient;
